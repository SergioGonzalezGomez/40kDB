...
const getUser = async (req, res, next) => {
    try {
        ...
        // }
        const userDB = await User.findById(id).populate('favoritelegions')
        
        if (!userDB) {
            return next(setError(404, 'User not found'))
        }
        return res.status(200).json({ name: userDB.name, email: userDB.email, emoji: userDB.emoji, favoritelegions: userDB.favoritelegions })
